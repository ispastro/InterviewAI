...
**InterviewMe** is a production-ready, full-stack AI-powered mock interview platform that provides real-time, personalized interview practice with intelligent feedback. The system follows a multi-agent architecture — separate reasoning roles handle question generation, answer evaluation, and session orchestration — implemented with custom logic rather than a third-party multi-agent framework.

### Key Features

- **Multi-Agent System**: Distinct agent roles for question generation, answer evaluation, and interview orchestration, built with custom orchestration logic (no external multi-agent framework)
